...
def index(request):
    featured_articles = Article.objects.filter(is_featured=True, category__slug__in=['academy', 'observatory']).order_by('-published_date')
    recent_articles = Article.objects.filter(category__slug__in=['academy', 'observatory']).order_by('-published_date')
